...
# v2ray_plugin_version="$(wget -qO- "https://521github.com/shadowsocks/v2ray-plugin/tags" | grep -E "/shadowsocks/v2ray-plugin/releases/tag/" | head -1 | sed -r 's/.*tag\/v(.+)\">.*/\1/')"
...
web_camouflage() {
    ...
    git clone https://521github.com/wulabing/3DCEList.git
    judge "web 站点伪装"
}
...
nginx_install() {
    ...
    judge "openssl 下载"
    wget -nc --no-check-certificate https://521github.com/jemalloc/jemalloc/releases/download/${jemalloc_version}/jemalloc-${jemalloc_version}.tar.bz2 -P ${nginx_openssl_src}
    judge "jemalloc 下载"
...
menu() {
    ...
    echo -e "\thttps://521github.com/wulabing\n"
